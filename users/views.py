...
def login_view(request):
    ...
    if request.method == 'POST' and 'login_submit' in request.POST:
        ...
        if not email or not password:
            ...
            if not user_exists:
                messages.error(request, "There is no active account associated with the email address.")
            else:
...
def register_view(request):
    ...
    if request.method == 'POST' and 'register_submit' in request.POST:
        if register_form.is_valid():
            ...
            user.username = user.email  # Set username to email to ensure uniqueness
            user.save()  # Now save to DB

            login(request, user, backend='users.backends.EmailBackend')
            messages.success(request, "Registered and logged in successfully!")
            return redirect('project_list')  # Redirect after registration
        else:
